...
export const responded: TData = {
  ...
  description:
    'Отклик отправлен успешно! Когда преподаватель примет заявку мы обменяем Вас контактами в Telegram, уведомление придёт автоматически через бота.Отменить или посмотреть статус заявки можно в разделе "Мои заявки"'
};
...
export const acceptRequest: TData = {
  title:
    'Мы обменяем Вас контактами с учеником, которые пришлёт бот через Telegram автоматически после нажатия кнопки <b>“Далее”</b>. После получения контакта ученика, пожалуйста, незамедлительно свяжитесь с ним.',
  variant: 'small'
};
