...
export const AuthSelectRole: FC<IAuthSelectRole> = ({ OnChangeSelect }) => {
    ...
    return (
        <div className={styles['container-selectors']}>
            <div onClick={() => setRoleSelected('pupil')} className={getRoleSelected === 'pupil' ? styles['active-select'] : ''}>
                <p>Как ученик</p>
            </div>
            <div onClick={() => setRoleSelected('teacher')} className={getRoleSelected === 'teacher' ? styles['active-select'] : ''}>
                <p>Как репетитор</p>
            </div>
        </div>
    )
